...
public abstract class BaseRestClientExpectTest<S> {
   ...
   protected Injector createInjector(Function<HttpRequest, HttpResponse> fn, Module module, Properties props) {
      ...
      if (builder == null){
         ...
         builder = pm != null ? ContextBuilder.newBuilder(pm) : ContextBuilder
               .newBuilder(ApiMetadata.class.cast(am));
      }
            
      this.api = builder.getApiMetadata().getApi();
      
      return builder.credentials(identity, credential)
            .modules(ImmutableSet.of(new ExpectModule(fn), new NullLoggingModule(), module))
