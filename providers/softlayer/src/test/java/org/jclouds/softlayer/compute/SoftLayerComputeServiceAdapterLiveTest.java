...
public class SoftLayerComputeServiceAdapterLiveTest extends BaseSoftLayerClientLiveTest {
   ...
   public void setupContext() {
      super.setupContext();
      adapter = wrapper.utils().injector().getInstance(SoftLayerComputeServiceAdapter.class);
   }
   ...
   protected void doConnectViaSsh(VirtualGuest guest, LoginCredentials creds) {
      SshClient ssh = wrapper.utils().sshFactory().create(new IPSocket(guest.getPrimaryIpAddress(), 22), creds);
      try {
         ssh.connect();
