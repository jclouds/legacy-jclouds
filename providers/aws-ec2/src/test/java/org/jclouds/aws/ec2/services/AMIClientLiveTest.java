...
public class AMIClientLiveTest extends BaseComputeServiceContextLiveTest {
   ...
   private Snapshot createSnapshot(ComputeService computeService) throws RunNodesException {
      ...
      try {
         ...
         Reservation<? extends RunningInstance> reservation = Iterables.getOnlyElement(wrapper.unwrap(AWSEC2ApiMetadata.CONTEXT_TOKEN).getApi().getInstanceServices().describeInstancesInRegion(null, instanceId));
         RunningInstance instance = Iterables.getOnlyElement(reservation);
         BlockDevice device = instance.getEbsBlockDevices().get("/dev/sda1");
         Snapshot snapshot = wrapper.unwrap(AWSEC2ApiMetadata.CONTEXT_TOKEN).getApi().getElasticBlockStoreServices().createSnapshotInRegion(null, device.getVolumeId());
   ...
   public void cleanUp() {
      ...
      for (String snapshotId : snapshotsToDelete)
         wrapper.unwrap(AWSEC2ApiMetadata.CONTEXT_TOKEN).getApi().getElasticBlockStoreServices().deleteSnapshotInRegion(null, snapshotId);
   }
