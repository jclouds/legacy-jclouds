...
public class PlacementGroupClientLiveTest extends BaseComputeServiceContextLiveTest {
   ...
   public void testStartCCInstance() throws Exception {
      ...
      Template template = wrapper.getComputeService().templateBuilder().fastest().osFamily(OsFamily.AMZN_LINUX).build();
      ...
      try {
         Set<? extends NodeMetadata> nodes = wrapper.getComputeService().createNodesInGroup(group, 1, template);
