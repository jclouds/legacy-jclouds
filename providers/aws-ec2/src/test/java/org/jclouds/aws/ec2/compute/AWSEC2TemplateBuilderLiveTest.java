...
public class AWSEC2TemplateBuilderLiveTest extends EC2TemplateBuilderLiveTest {
   ...
   public void testTemplateBuilderM1MEDIUMWithNegativeLookaroundDoesntMatchTestImages() {
      ...
      Template template = wrapper.getComputeService().templateBuilder().hardwareId(InstanceType.M1_MEDIUM)
   ...
   public void testTemplateBuilderCanUseImageIdAndhardwareIdAndAZ() {
      ...
      Template template = wrapper.getComputeService().templateBuilder().imageId("us-east-1/ami-ccb35ea5")
   ...
   public void testAmazonLinuxInstanceStore() throws IOException {
      ...
      Template defaultTemplate = wrapper.getComputeService().templateBuilder().osFamily(OsFamily.AMZN_LINUX)
   ...
   public void testFastestTemplateBuilder() throws IOException {
      Template fastestTemplate = wrapper.getComputeService().templateBuilder().fastest().osFamily(OsFamily.AMZN_LINUX)
            .build();
      ...
      fastestTemplate = wrapper.getComputeService().templateBuilder().fastest().build();
   ...
   public void testTemplateBuilderMicro() throws IOException {
      ...
      Template microTemplate = wrapper.getComputeService().templateBuilder().hardwareId(InstanceType.T1_MICRO)
   ...
   public void testTemplateBuilderWithLessRegions() throws IOException, SecurityException, NoSuchMethodException {
      ...
      try {
         ...
         assert context.getComputeService().listAssignableLocations().size() < this.wrapper.getComputeService()
               .listAssignableLocations().size();

         assertOnlyOneRegionQueriedForAvailabilityZone(commandsInvoked);

         assert context.getComputeService().listImages().size() < this.wrapper.getComputeService().listImages().size();
   ...
   public void testTemplateBuilderCanUseImageIdFromNonDefaultOwner() {
      ...
      Template defaultTemplate = wrapper.getComputeService().templateBuilder().imageId(imageId)
            .imageMatches(EC2ImagePredicates.rootDeviceType(RootDeviceType.INSTANCE_STORE)).build();
      assert (defaultTemplate.getImage().getProviderId().startsWith("ami-")) : defaultTemplate;
   ...
   public void testAssignability() {
      wrapper.unwrap(EC2ApiMetadata.CONTEXT_TOKEN);
      wrapper.unwrap(AWSEC2ApiMetadata.CONTEXT_TOKEN);
   }
