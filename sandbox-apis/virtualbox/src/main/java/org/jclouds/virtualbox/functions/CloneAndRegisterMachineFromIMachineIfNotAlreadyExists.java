...
public class CloneAndRegisterMachineFromIMachineIfNotAlreadyExists implements
   ...
   private IMachine cloneMachine(IVirtualBox vBox, String cloneName,
      ...
      // clone
      IProgress progress = master.getCurrentSnapshot().getMachine()
            .cloneTo(clonedMachine, CloneMode.MachineState, options);
   ...
   protected static String retrieveAvailableBridgedInterfaceInfo(
      ...
      for (String bridgedIf : Splitter.on(Pattern.compile("(?m)^[ \t]*\r?\n"))
         ...
         Iterable<String> block = Iterables.filter(
               Splitter.on("\n").split(bridgedIf), new Predicate<String>() {
                  @Override
                  public boolean apply(String arg0) {
                     return arg0.startsWith("Name:")
                           || arg0.startsWith("IPAddress:")
                           || arg0.startsWith("Status:");
                  }
               });
       ...
       * if(!bridgedIf.isEmpty()) bridgedInterfaces.add(new
       * BridgedInterface(bridgedIf));
       * 
       * } for (BridgedInterface bridgedInterface : bridgedInterfaces) {
       * if(bridgedInterface.getStatus().equals("Up")) return bridgedInterface;
       * }
       */
