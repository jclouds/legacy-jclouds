...
import static org.jclouds.virtualbox.functions.IsoToIMachine.lockSessionOnMachineAndApply;
...
public class IMachineToIpAddressLiveTest extends BaseVirtualBoxClientLiveTest {
   ...
   private String masterName = "jclouds-image-virtualbox-iso-to-machine-test";
   private String cloneName = "jclouds-image-virtualbox-machine-to-machine-test_clone";
   ...
   public void testConvert() throws IOException {
      ...
      IMachine master =       
            new IsoToIMachine(manager, adminDisk, diskFormat,
            settingsFile, masterName, osTypeId, vmId, forceOverwrite,
      ...
      IMachine clone = new CloneAndRegisterMachineFromIMachineIfNotAlreadyExists(
            manager, localContext, "", "", "", false, cloneName, hostId)
            .apply(master);

     
      // TODO this should wait for vm readiness
      new LaunchMachineIfNotAlreadyRunning(manager, ExecutionType.GUI, "").apply(clone);

      String ipAddress = new IMachineToIpAddress(localContext, hostId, network)
            .apply(clone);
      assertTrue(!ipAddress.isEmpty());
      
      lockSessionOnMachineAndApply(manager, Shared, cloneName, new Function<ISession, Void>() {
