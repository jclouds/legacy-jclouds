...
public class DataObject extends CDMIObject {
	 ...
	 * @return value
	 */
	public InputStream getValueAsInputStream(Charset charset) {
	...
	public File getValueAsFile(File destDir) throws IOException {
		...
		Files.copyFile(getValueAsInputStream(), fileOut);
		return fileOut;
	}
	...
	public File getValueAsFile(File destDir, Charset charset)
			throws IOException {
		File fileOut = new File(destDir, this.getObjectName());
		Files.copyFile(getValueAsInputStream(charset), fileOut);
		return fileOut;
	}
