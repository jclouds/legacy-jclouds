...
public interface BuiltinServerAsyncApi {
    ...
    @GET
    @JAXBResponseParser
    @QueryParams(keys = "Action", values = "GetEFMAttribute")
    @Transform(SingleElementResponseToElement.class)
    ListenableFuture<BuiltinServer> get(
            @BinderParam(BindAlsoToSystemId.class) @QueryParam("efmId") String id);

    // getConfiguration(@BinderParam(BindAlsoToSystemId.class)
    // @QueryParam("efmId") String id);
    ...
    // Set<> listEFMBackup(@BinderParam(BindAlsoToSystemId.class)
    // @QueryParam("efmId") String id);
}
