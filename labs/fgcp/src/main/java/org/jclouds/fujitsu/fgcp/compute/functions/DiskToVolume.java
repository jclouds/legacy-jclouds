...
public class DiskToVolume implements Function<Disk, Volume> {
    ...
    @Override
    public Volume apply(Disk from) {
        VolumeBuilder builder = new VolumeBuilder();

        builder.size(1000f * Float.valueOf(from.getSize()));
        // "Disk"'s are additional disks; they can't be booted from(?)
        builder.bootDevice(false);
        builder.durable(true);
        builder.type(Volume.Type.SAN);
        builder.id("type: " + from.getType() + " usage: " + from.getUsage());
