...
public class AptInstaller implements PackageProcessor {

   private final ComputeServiceContext ctx;

   @Inject
   public AptInstaller(ComputeServiceContext ctx) {
      this.ctx = ctx;
   }
   ...
   public static class AptInstallStatement implements Statement {
      ...
      public String render(org.jclouds.scriptbuilder.domain.OsFamily family) {
         StringBuilder builder = new StringBuilder().append("apt-get -y install");
