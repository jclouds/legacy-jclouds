...
public class KeyPairsApiLiveTest extends BaseVCloudDirectorApiLiveTest {
	...
	public void testCreateKeyPair() throws URISyntaxException {
		...
		MetadataValue keypairValue = mediaApi.getMetadataApi(keyPairsContainer.getId()).getValue(keyPairName);
		assertEquals(keypairValue.getValue(), generateKeyPair(keyPairName).getValue());
	}
	...
	private Media uploadKeyPairInVCD(Vdc currentVDC,
		...
		assertTrue(keyPairsContainer.getFiles().size() == 1, String.format(
				OBJ_FIELD_LIST_SIZE_EQ, MEDIA, "files", 1, keyPairsContainer.getFiles()
						.size()));
	...
	private Media generateEmptyMediaInVDC(Vdc currentVDC, String keyPairName) {
		Link addMedia = find(
				currentVDC.getLinks(),
				and(relEquals("add"),
						typeEquals(VCloudDirectorMediaType.MEDIA)));
		
		Media sourceMedia = Media.builder()
				.type(VCloudDirectorMediaType.MEDIA).name(keyPairName)
				.size(0).imageType(Media.ImageType.ISO)
	...
	private MetadataValue generateKeyPair(String keyPairName) {
		...
		key.put("keyFingerprint",
				SshKeys.sha1PrivateKey(sshKey.get("private")));
		key.put("publicKey", sshKey.get("public"));

		MetadataValue keyPair = MetadataValue.builder()
				.value(json.toJson(key)).build();
		return keyPair;
	}

	private void setKeyPairOnkeyPairsContainer(Media media, String keyPairName, MetadataValue keyPair) {
		Task setKeyPair = mediaApi.getMetadataApi(media.getId())
				.putEntry(keyPairName, keyPair);
