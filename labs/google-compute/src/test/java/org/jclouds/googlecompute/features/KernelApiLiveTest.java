...
public class KernelApiLiveTest extends BaseGoogleComputeApiLiveTest {
   ...
   private KernelApi api() {
      return context.getApi().getKernelApi();
   }

   @Test(groups = "live")
   public void testListKernel() {

      PagedIterable<Kernel> kernels = api().list(GOOGLE_PROJECT, ListOptions.builder()
              .maxResults(1)
              .build());
   ...
   public void testGetKernel() {
      Kernel kernel = api().get(GOOGLE_PROJECT, this.kernel.getName());
