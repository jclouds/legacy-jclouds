...
public class MachineTypeApiLiveTest extends BaseGoogleComputeApiLiveTest {
   ...
   private MachineTypeApi api() {
      return context.getApi().getMachineTypeApi();
   }

   @Test(groups = "live")
   public void testListMachineType() {

      PagedIterable<MachineType> machineTypes = api().list(GOOGLE_PROJECT, ListOptions.builder()
              .maxResults(1)
              .build());
   ...
   public void testGetMachineType() {
      MachineType machineType = api().get(GOOGLE_PROJECT, this.machineType.getName());
