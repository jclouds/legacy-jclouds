...
public class OperationApiLiveTest extends BaseGoogleComputeApiLiveTest {
   ...
   private OperationApi api() {
      return context.getApi().getOperationApi();
   }
   ...
   public void testCreateOperations() {
      ...
      // this will make sure there is stuff to list
      ...
      deleteOperation = assertOperationDoneSucessfully(deleteItemFromMetadata(context.getApi().getProjectApi(),
              getUserProject(), METADATA_ITEM_KEY), 20);
   ...
   public void testGetOperation() {
      Operation operation = api().get(getUserProject(), addOperation.getName());
   ...
   public void testListOperationsWithFiltersAndPagination() {
      PagedIterable<Operation> operations = api().list(getUserProject(), ListOptions.builder()
