...
public interface ImageAsyncApi {
   ...
   /**
    * @see ImageApi#get(String, String)
    */
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/images/{image}")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @ExceptionParser(ReturnNullOnNotFoundOr404.class)
   public ListenableFuture<Image> get(@PathParam("project") String projectName,
                                      @PathParam("image") String imageName);

   /**
    * @see ImageApi#insert(String, org.jclouds.googlecompute.domain.Image)
    */
   @POST
   @Consumes(MediaType.APPLICATION_JSON)
   ...
   public ListenableFuture<Operation> insert(@PathParam("project") String projectName,
                                             @BinderParam(BindToJsonPayload.class) Image image);

   /**
    * @see ImageApi#delete(String, String)
    */
   @DELETE
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/images/{image}")
   ...
   /**
    * @see ImageApi#list(String, org.jclouds.googlecompute.options.ListOptions)
    */
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/images")
   ...
   @ResponseParser(ParsePagedLists.ParseImages.class)
   @ExceptionParser(ReturnEmptyPagedIterableOnNotFoundOr404.class)
   public ListenableFuture<? extends PagedIterable<Image>> list(@PathParam("project") String projectName,
                                                                ListOptions options);
}
