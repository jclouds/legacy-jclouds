...
public interface KernelAsyncApi {
   ...
   /**
    * @see KernelApi#get(String, String)
    */
   @GET
   @Path("/projects/{project}/kernels/{kernel}")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @ExceptionParser(ReturnNullOnNotFoundOr404.class)
   public ListenableFuture<Kernel> get(@PathParam("project") String projectName,
                                       @PathParam("kernel") String kernelName);

   /**
    * @see KernelApi#list(String, org.jclouds.googlecompute.options.ListOptions)
    */
   @GET
   @Path("/projects/{project}/kernels")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @Transform(ToPagedIterable.class)
   @ResponseParser(ParsePagedLists.ParseKernels.class)
   @ExceptionParser(ReturnEmptyPagedIterableOnNotFoundOr404.class)
   public ListenableFuture<? extends PagedIterable<Kernel>> list(@PathParam("project") String projectName,
