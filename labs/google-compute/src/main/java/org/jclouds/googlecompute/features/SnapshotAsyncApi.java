...
/**
 * Provides asynchronous access to Snapshots via their REST API.
 *
 * @author David Alves
 * @see SnapshotApi
 *
...
public interface SnapshotAsyncApi {
   ...
   /**
    * @see SnapshotApi#get(String, String)
    */
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/snapshots/{snapshot}")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @ExceptionParser(ReturnNullOnNotFoundOr404.class)
   public ListenableFuture<Snapshot> get(@PathParam("project") String projectName,
                                         @PathParam("snapshot") String snapshotName);

   /**
    * @see SnapshotApi#insert(String, org.jclouds.googlecompute.domain.Snapshot)
    */
   @POST
   @Consumes(MediaType.APPLICATION_JSON)
   @Produces(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/snapshots")
   @OAuthScopes({COMPUTE_SCOPE})
   public ListenableFuture<Operation> insert(@PathParam("project") String projectName,
                                             @BinderParam(BindToJsonPayload.class) Snapshot snapshot);

   /**
    * @see SnapshotApi#delete(String, String)
    */
   @DELETE
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/snapshots/{snapshot}")
   ...
   /**
    * @see SnapshotApi#list(String, org.jclouds.googlecompute.options.ListOptions)
    */
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/projects/{project}/snapshots")
   ...
   @ResponseParser(ParsePagedLists.ParseSnapshots.class)
   @ExceptionParser(ReturnEmptyPagedIterableOnNotFoundOr404.class)
   public ListenableFuture<? extends PagedIterable<Snapshot>> list(@PathParam("project") String projectName,
                                                                   ListOptions options);
