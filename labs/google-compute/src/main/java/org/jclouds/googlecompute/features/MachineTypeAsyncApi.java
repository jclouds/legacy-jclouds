...
public interface MachineTypeAsyncApi {
   ...
   /**
    * @see MachineTypeApi#get(String, String)
    */
   @GET
   @Path("/projects/{project}/machineTypes/{machineType}")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @ExceptionParser(ReturnNullOnNotFoundOr404.class)
   public ListenableFuture<MachineType> get(@PathParam("project") String projectName,
                                            @PathParam("machineType") String machineTypeName);

   /**
    * @see MachineTypeApi#list(String, org.jclouds.googlecompute.options.ListOptions)
    */
   @GET
   @Path("/projects/{project}/machineTypes")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @Transform(ToPagedIterable.class)
   @ResponseParser(ParsePagedLists.ParseMachineTypes.class)
   @ExceptionParser(ReturnEmptyPagedIterableOnNotFoundOr404.class)
   public ListenableFuture<? extends PagedIterable<MachineType>> list(@PathParam("project") String projectName,
