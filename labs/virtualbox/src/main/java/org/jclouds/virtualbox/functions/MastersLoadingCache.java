...
<<<<<<< HEAD
   ...
   public MastersLoadingCache(@BuildVersion String version,
            ...
            Function<URI, File> isoDownloader, Factory runScriptOnNodeFactory, RetryIfSocketNotYetOpen socketTester, Supplier<NodeMetadata> host,
   ...
   private MasterSpec buildMasterSpecFromYaml(YamlImage currentImage, String vmName) throws ExecutionException {
      ...
      NetworkAdapter networkAdapter = NetworkAdapter.builder().networkAttachmentType(NetworkAttachmentType.NAT)
               .tcpRedirectRule(providerSupplier.get().getHost(), MASTER_PORT, "", 22).build();
...
//          logger.debug("Check %s file is present", file);
//          if (!file.exists()) {
//             logger.debug("iso not found in cache, downloading: %s", httpUrl);
//             file = isoDownloader.apply(URI.create(httpUrl));
//          }
//          checkState(file.exists(), "iso file has not been downloaded: " + fileName);
      ...
      NodeMetadata hostNodeMetadata = NodeMetadataBuilder.fromNodeMetadata(host.get())
    		  .credentials(LoginCredentials.builder().user("toor").password("password").authenticateSudo(true).build())
    		  .publicAddresses(ImmutableList.of(providerSupplier.get().getHost()))
    		  .build();
          runScriptOnNodeFactory.create(hostNodeMetadata, statementList,
