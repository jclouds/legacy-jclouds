...
public class CreateDeployment {
	...
	/**
	 * The name for the deployment. The deployment name must be unique among
	 * other deployments for the hosted service.
	 */
	@XmlElement(required = true, name = "Name")
	private String name;

	/**
	 * A URL that refers to the location of the service package in the Blob
	 * service. The service package can be located either in a storage account
	 * beneath the same subscription or a Shared Access Signature (SAS) URI from
	 * any storage account.
	 */
	 ...
	 * 
	 * If false, the service model is still deployed to the virtual machines but
	 * the code is not run immediately. Instead, the service is Suspended until
	 * you call Update Deployment Status and set the status to Running, at which
	 * time the service will be started. A deployed service still incurs
	 * charges, even if it is suspended.
	 */
	...
	public int hashCode() {
		...
		result = prime * result
				+ ((packageUrl == null) ? 0 : packageUrl.hashCode());
		result = prime * result
				+ ((startDeployment == null) ? 0 : startDeployment.hashCode());
		result = prime
				* result
				+ ((treatWarningsAsError == null) ? 0 : treatWarningsAsError
						.hashCode());
		return result;
	}
	...
	public String toString() {
		return "CreateDeployment [name=" + name + ", packageUrl=" + packageUrl
